...
graph <- leiden_workflow(graph, res_2 = 0.2)
...
#########################################################################################################################
#########################################################################################################################
# We extract keywords from title ref
#########################################################################################################################
#########################################################################################################################
...
mystopwords <- tibble(word = c(comparaison,"TAPIERO", "RAJAN","SCI","992","1328","1109", "1116","410","LYON","1315","HOEHN","HAWAII"))
...
ggraph(graph, "manual", x = x, y = y) + 
  ...
  new_scale("size") +
  geom_text_repel(data=top_nodes, aes(x=x, y=y, label = Label), size = 2, fontface="bold", alpha = 1, point.padding=NA, show.legend = FALSE) +
  geom_label_repel(data=community_labels2[community_labels2$Size_com>0.015,], aes(x=x, y=y, label = Community_name, fill = color, size = Size_com), fontface="bold", alpha = 0.9, point.padding=NA, show.legend = FALSE) +
  scale_size_continuous(range = c(0.5,5)) +
  theme_void()
